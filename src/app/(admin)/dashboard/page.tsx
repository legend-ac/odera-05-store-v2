    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href="/dashboard/products" className="text-sm px-3 py-2 rounded-lg border border-neutral-300 bg-white hover:bg-neutral-50">
            Productos
          </Link>
          <Link href="/dashboard/orders" className="text-sm px-3 py-2 rounded-lg border border-neutral-300 bg-white hover:bg-neutral-50">
            Pedidos
          </Link>
        </div>
      </div>
      ...
      <div className="border border-neutral-200 rounded-xl overflow-hidden bg-white">
        <div className="px-4 py-3 bg-neutral-50 text-sm font-medium">Ultimos pedidos</div>
          ...
          {orders.map((o) => (
            <div key={o.id} className="px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div>
                ...
                <div className="text-xs text-neutral-600">{o.email ?? "-"}</div>
              </div>
              <div className="text-left sm:text-right">
                <div className="text-xs inline-flex px-2 py-1 rounded-md bg-slate-100 text-slate-700">
                  {STATUS_LABEL[o.status] ?? o.status}
                </div>
                <div className="text-xs text-neutral-600 mt-1">{o.createdAt}</div>
              </div>
            </div>
          ))}
          {!orders.length ? <div className="px-4 py-4 text-sm text-neutral-600">Sin pedidos.</div> : null}
        </div>
      </div>
