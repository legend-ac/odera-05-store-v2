...
export async function POST(req: Request) {
  try {
    ...
    const a = await processExpiredForStatus("SCHEDULED", now, 50);
    const b = await processExpiredForStatus("PAYMENT_SENT", now, 50);

    return NextResponse.json({ ok: true, processed: a + b }, { status: 200 });
  } catch (e) {
