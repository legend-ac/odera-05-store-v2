...
export async function POST(req: Request) {
  try {
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      if (!before?.deletedAt) throw new Error("ORDER_PURGE_REQUIRES_TRASH");
      ...
      tx.set(auditRef, {
        ...
        before: { status: before?.status ?? "", deletedAt: before?.deletedAt ?? null },
        after: null,
    ...
    const codeToStatus: Record<string, number> = {
      ...
      AUTH_TOO_OLD: 401,
      ORDER_NOT_FOUND: 404,
      ORDER_PURGE_REQUIRES_TRASH: 409,
    };
